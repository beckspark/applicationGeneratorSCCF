...
def approve_client():
    global approval, hourly, percent, denture, dentures, partial, valplast, deep_poverty
    poverty_lookup()
    if status == '1':
        poverty = poverty_1
    elif status == '2':
    ...
    if approval != 'Denial':
        approval = 'Approval'
    if aincome <= deep_poverty:
        deep_poverty = 1
    elif aincome > deep_poverty:
        deep_poverty = 0


def housing_lookup():
    global housing, address, city, zip_code, housing_type, county
    db = pd.read_csv(os.path.join('processing','senior_housing.csv'))
...
def data_to_csv(d, t):
    df_csv = os.path.join('processing',(t+'.csv'))
...
def format_data():
    ...
    find_age()
    approve_client()
    sccf_clients_clients = {'apply_date':[apply_date], 'approval_date':[approval_date], 'lname':[lname],'fname':[fname], 'full_name':[full_name], 'initials':[initials], 'service':[service], 'hourly':[hourly], 'phone':[phone], 'housing':[housing], 'address':[address], 'city':[city], 'unit':[unit], 'zip_code':[zip_code], 'county':[county], 'housing_type':[housing_type], 'approval':[approval], 'percent':[percent], 'denture':[denture], 'dentures':[dentures], 'partial':[partial], 'valplast':[valplast], 'dob':[dob]}
    sccf_clients_demographics = {'full_name':full_name, 'sex':sex, 'age':age, 'resident':resident, 'zip_code':zip_code, 'edu':edu, 'aincome':aincome, 'deep_poverty':deep_poverty, 'dental_insurance':dental_insurance, 'ins_type':ins_type, 'medicaid':medicaid, 'disability':disability, 'dis_type':dis_type, 'race':race, "lang":lang}
